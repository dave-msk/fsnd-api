...
class TriviaTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.database_url = "david:sqlDev@localhost:5432"
  ...
  def validate_response(self, res, code, types, allow_none=False):
    ...
    for k in types:
      v = data[k]
      if allow_none and v is None: continue
  ...
  def testGetFirstQuizQuestion(self):
    ...
    data = self.validate_response(res, 200, {"success": bool, "question": dict})
    self.assertEqual(data["success"], True)
    rq = data["question"]
