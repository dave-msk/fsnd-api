...
def valid_and_cast(data, types, optional=None, cast=True):
  ...
  if missing: fsk.abort(400)

  unknowns = set(data) - set(types)
  if unknowns: fsk.abort(400)
    ...
    if k in data:
      ...
      if v is not None and cast:
        try:
          ...
          pass
      if not isinstance(v, t): fsk.abort(400)
      out[k] = v
...
def create_app(test_config=None):
  ...
  # 0. Use the after_request decorator to set Access-Control-Allow
  #   Allow '*' for origins.
  ...
  # 1. Create endpoints
  # 1.1 Create an endpoint to handle GET requests for all available categories.
  ...
  # 1.2 Create an endpoint to get questions based on category.
  ...
  # 1.3 Create a POST endpoint to get questions based on a search term.
  ...
  def search_questions():
    ...
    query = models.Question.query.order_by(models.Question.id)
    if search_term:
      query = query.filter(models.Question.question.ilike(f"%{search_term}%"))
    count = query.count()
  ...
  # 1.4 Create an endpoint to POST a new question, which will require the
  ...
  # 1.7 Create a POST endpoint to get questions to play the quiz.
  ...
  def get_quiz_question():
    ...
    if cid != 0: query = query.filter(models.Question.category == cid)
    qids = [q.id for q in query]

    q = None
    if qids:
      qid = qids[random.randint(0, len(qids)-1)]  # Choose question randomly
      q = models.Question.query.get(qid).format()  # Get chosen question
    return fsk.jsonify({"success": True, "question": q})

  # 2. Create error handlers for all expected errors
  # 2.1 Create error handler for status 400
  @app.errorhandler(400)
  def bad_request_error(error):
    return error_json(400, "bad request"), 400

  # 2.2 Create error handler for status 404
  @app.errorhandler(404)
  def not_found_error(error):
    return error_json(404, "resource not found"), 404

  # 2.3 Create error handler for status 422
  @app.errorhandler(422)
  def unprocessable_error(error):
    return error_json(422, "unprocessable"), 422

  # 2.4 Create error handler for status 500
